...
import com.usrivastava.model.CreateImageEditRequestModel;
import java.io.File;
...
public class CreateImageEditRequest {
  ...
  * @throws IOException if the JSON Element is invalid with respect to CreateImageEditRequest
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CreateImageEditRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in CreateImageEditRequest is not found in the empty JSON string", CreateImageEditRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CreateImageEditRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CreateImageEditRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : CreateImageEditRequest.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CreateImageEditRequest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CreateImageEditRequest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CreateImageEditRequest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CreateImageEditRequest.class));

       return (TypeAdapter<T>) new TypeAdapter<CreateImageEditRequest>() {
           @Override
           public void write(JsonWriter out, CreateImageEditRequest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CreateImageEditRequest read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of CreateImageEditRequest given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of CreateImageEditRequest
  * @throws IOException if the JSON string is invalid with respect to CreateImageEditRequest
  */
  public static CreateImageEditRequest fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CreateImageEditRequest.class);
  }

 /**
  * Convert an instance of CreateImageEditRequest to an JSON string
  *
  * @return JSON string
  */
